...
uniform vec4 colorHDR1;
uniform vec4 colorHDR2;
// uniform vec4 color2;
...
void mainImage(out vec4 fragColor, in vec2 uv) {
    vec4 lerp = vec4(mix(colorHDR1, colorHDR2, uv.x));
    ...
    vec3 tail = texture(TailTexture, uv + tailOffset).rgb * mask;
    float a = pow(max(tail.r, max(tail.g, tail.b)), 1.4);
    fragColor = vec4(lerp.rgb * lerp.a * tail, clamp(a, 0, 1));
}
